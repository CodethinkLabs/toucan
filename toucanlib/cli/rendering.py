...
class ObjectClassShowRenderer(object):
    ...
    def _list_comments(self, comments, lines, n_length):
        ...
        for comment in comments:
            ...
            comment_ID = str(
                comment.properties['number'].value).ljust(n_length)
            comment_content = comment.properties['comment'].value
            user_ref = comment.properties['author'].value
            ...
            indent = 13 + n_length
            wrapper = textwrap.TextWrapper(
                        initial_indent=(' ' * indent) + '# ', width=80)
    ...
    def _get_lane(self, lane, name_length):
        ...
        if 'cards' in lane.properties:
            cards = '%d cards' % len(lane.properties['cards'].value)
        else:
            cards = '0 cards'
    ...
    def _get_attachment(self, attachment):
        ...
        # get required information
        attachment_name = attachment.properties['name'].value
        content_type = attachment.properties['content-type'].value

        # return the formatted string
        return 'attachment/%s # %s' % (attachment_name.lower(), content_type)
    ...
    def _resolve_references(self, refs, prop=''):
        ...
        for ref in refs:
            ...
            if (prop != ''):
                length = max(len(str(
                            objects[-1].properties[prop].value)), length)
    ...
    def render(self, stream, objects):
        ...
        for obj in objects:
            ...
            if 'views' in obj.properties:
                lines.append('views:')
                name_length, views = self._resolve_references(
                                obj.properties['views'].value, 'name')
...
class AttachmentShowRenderer(ObjectClassShowRenderer):
    ...
    def render(self, stream, objects):
        ...
        for obj in objects:
            ...
            n_length, comments = self._resolve_references(
                            [obj.properties['comment']], 'number')
...
class ViewShowRenderer(ObjectClassShowRenderer):
    ...
    def render(self, stream, objects):
        ...
        for obj in objects:
            ...
            name_length, lanes = self._resolve_references(
                            obj.properties['lanes'].value, 'name')
    ...
    def render(self, stream, objects):
        ...
        for obj in objects:
            ...
            if 'cards' in obj.properties:
                ...
                n_length, cards = self._resolve_references(
                            obj.properties['cards'].value, 'number')
...
class CardShowRenderer(ObjectClassShowRenderer):
    ...
    def render(self, stream, objects):
        ...
        for obj in objects:
            ...
            lines.append('title: %s' % (obj.properties['title'].value))
            ...
            length, lane = self._resolve_references([obj.properties['lane']])
            lines.append('lane: ' + self._get_lane(lane[0], length))
            ...
            if 'assignees' in obj.properties:
                lines.append('assignees:')
                name_length, assignees = self._resolve_references(
                                obj.properties['assignees'].value)
                for user in assignees:
                    name_length = max(len(
            ...
            if 'comments' in obj.properties:
                lines.append('comments:')
                n_length, comments = self._resolve_references(
                                obj.properties['comments'].value, 'number')
...
class MilestoneShowRenderer(ObjectClassShowRenderer):
    ...
    def render(self, stream, objects):
        ...
        for obj in objects:
            ...
            d = obj.properties['deadline'].value.value.date().isoformat()
...
class CommentShowRenderer(ObjectClassShowRenderer):
    ...
    def render(self, stream, objects):
        ...
        for obj in objects:
            ...
            # next, the author
            author = self.service.resolve_reference(
                        obj.properties['author'].value)
            ...
            if 'attachment' in obj.properties:
                attachment = self.service.resolve_reference(
                            obj.properties['attachment'].value)
                lines.append('attachment: %s'
                            % (self._get_attachment(attachment)))
            lines.append('---')
    ...
    def _render_comment(self, obj, lines):
        ...
        lines += wrapper.wrap(obj.properties['comment'].value)
...
class ShowRenderer(object):
    ...
    def _render_group(self, stream, name, objects):
        ...
        klass(self.service).render(stream, objects)
