...
class ObjectLoader():
    ...
    def _update_view(self, action_id):
        ...
        for lane in lanes:
            ...
            lane_props = [properties.ListProperty('views', lane['views'])]
    ...
    def _update_lane(self, action_id):
        ...
        for view in views:
            ...
            view_props = [properties.ListProperty('lanes', view['lanes'])]
            acts.append(actions.UpdateAction(
    ...
    def _update_card(self, action_id):
        ...
        # add lane reference
        lanes = self.resolver.resolve_patterns(card['lane'].lower(), 'lane')
        lane = lanes.pop()
    ...
    def _validate_attachment(self, phase):
        ...
        if not isinstance(attachment, dict):
            ...
            if not 'comment' in attachment:
                ...
                matches = self.resolver.resolve_patterns(
                    attachment['comment'], 'comment')
                if not matches:
                    phase.error(ObjectLoaderError(
        ...
        if not 'creator' in card:
            ...
            if not matches:
                phase.error(ObjectLoaderError(
                    ...
                    'non-existent user: %s' % card['creator']))
                return False
        return True
                ...
                if not isinstance(lane['cards'], list):
                    ...
                    for card in lane['cards']:
                        if not isinstance(card, int):
                            ...
                            return False
                        matches = self.resolver.resolve_patterns(card, 'card')
                        if not matches:
                            phase.error(ObjectLoaderError(
            ...
            if 'default-view' in user:
                if not isinstance(user['default-view'], basestring):
                    phase.error(ObjectLoaderError(
                        ...
                        'default-view: %s' % user['default-view']))
                    return False
                else:
                    matches = self.resolve_patterns(
                        user['default-view'].lower(), 'view')
                    if not matches:
                        phase.error(ObjectLoaderError(
                            'Input file defines a user that refers to a '
                            'non-existent default-view: %s' %
                            user['default-view']))
                        return False
            ...
            if not 'lanes' in view:
                ...
                for lane in view['lanes']:
                    if not isinstance(lane, basestring):
                        ...
                        matches = self.resolver.resolve_patterns(lane, 'lane')
                        if not matches:
                            phase.error(ObjectLoaderError(
...
class Phase(consonant.util.phase.Phase):

    """Error tracker for the validator."""

    def __exit__(self, *args):
        """Exit silently without raising exceptions.

        Errors are printed to the data file and progress cannot be made whilst
        there are errors.
